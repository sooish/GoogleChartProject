...
‘서울시 열린데이터 광장’에서 제공하는 [서울시 개인적인 고민거리 통계](https://data.seoul.go.kr/dataList/datasetView.do?infId=10416&srvType=S&serviceKind=2&currentPageNo=1&searchValue=&searchKey=null) (서울특별시 스마트도시정책관 빅데이터담당관, 2014년) 공공데이터를 활용하여, 차트와 워드클라우드로 데이터 시각화를 시도해보는 프로젝트이다. 
...
제공받은 데이터를 가공하여 개인의 고민 종류를 연령/ 성별 / 소득 / 혼인상태 / 학력 / 지역(지역구)이라는 범주로 나누어 살펴본다. 여러가지 범주 가운데, 개인의 고민에 있어 가장 큰 상관관계를 보이는 것은 '연령'일 것이라는 가설이 참이라는 것을 확인할 수 있다. 즉, 성별, 소득. 혼인상태, 학력, 지역에 따른 차이가 미미한 개인의 고민이, 연령에 따라서는 확연한 차이를 보인다. 특히, 혼인상태에 따른 개인별 고민의 차이도 혼인상태의 직접적인 영향이라기 보다 연령이라는 제3의 변수(교랸요인)에 매개되고 있음을 유추해 볼 수 있다. 또 한가지 흥미로운 점은, 개인의 고민이 학력에 따라 큰 차이를 보이지는 않는 한편, 유독 중졸 이하에서만 '건강'이 독보적인 고민거리로 부상된다는 사실이다. 이것은 소득이 100만원 이하인 경우에 나타나는 것과 비슷한 양상으로서, 중졸이하에서는 학력이 절대적 빈곤과 밀접한 관련이 있기 때문일 것이라고 유추해 볼 수 있다.
